...
class HttpRequest {
	...
	public:
		...
		struct Request {
			...
			Request(MethodType type = MethodType::UNKOWN_METHOD, std::string url = "", std::vector<std::string> param = {},
				std::string version = "", std::vector<Header> header = {}, Body body = Body{}) 
					: method(type), url(url), params(params), version(version), headers(header), body(body) { }
		};
